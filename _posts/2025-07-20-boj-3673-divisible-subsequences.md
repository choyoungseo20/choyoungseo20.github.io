...
![](/assets/img/posts/2025-07-20-백준c-3673번-나눌-수-있는-부분-수열-01.png)

<https://www.acmicpc.net/problem/3673>

## **풀이**

1. 수열의 원소를 입력받을 때, 해당 원소까지의 합을 d로 나눈 나머지를 저장한다. 합을 그대로 저장할 경우 sum이 너무 커지기에 비효율적이다.

2. d로 나눈 나머지가 나온 개수를 추가로 저장한다.

3. 같은 값을 가진 두 나머지가 있을 경우, 그 사이의 합은 d로 나누어 떨어진다. (예시 참고)

4. 같은 값을 가진 나머지가 n개일 경우 그 중 두 개(부분 수열의 시작점, 끝점)를 선택하면 되기에 조합(nC2)을 사용한다.

예시  
1) d = 3이고, {1 4 2} 수열이 있을 경우   
